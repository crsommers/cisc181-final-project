...
export class ActionMove extends Action {
    ...
    /* Direct Getters */
    validAction(): boolean { return this.game.getRules().checkValidMove(this.startLocation, this.endLocation); }
    ...
    performAction(): void {
        ...
        if (piece) {
            if (!endSquare.isCracked()) endSquare.setPiece(piece);
            else this.game.getCurrentTeam().removePieceFromTeam(piece);
