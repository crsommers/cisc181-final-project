...
export class ActionSpawn extends Action {
    ...
    /* Direct Getters */
    validAction(): boolean { return this.game.getRules().checkValidSpawn(this.startLocation, this.endLocation); }
    ...
    performAction(): void {
        ...
        if (piece) {
            const spawned = piece.spawn();
            if (!endSquare.isCracked()) {
                endSquare.setPiece(spawned);
                this.game.getCurrentTeam().addPieceToTeam(spawned);
            }
            spawned.speak();
